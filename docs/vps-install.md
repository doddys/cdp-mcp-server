...
Once installed (§1–§6), shipping a new commit to `main` is pull + restart —
...
- `cdp-mcp-autossh.service` and `cdp-mcp-kinit.timer` are independent units —
  a `cdp-mcp.service` restart doesn't touch the tunnel or the TGT, so B1
  Kerberos state survives untouched. If you also need to bump those (new jump
  host, rotated keytab), restart them separately.
